...
async function getCommonInputs() {
  ...
  result.repositories = core.getMultilineInput("repositories", {
    required: true,
  });
...
async function checkoutRepos() {
  const ci = await getCommonInputs();
  const promises2 = [];
  for (let repository of ci.repositories) {
    if (repository.split("|").length < 3) {
      core2.setFailed(`Not enough arguments for repositories`);
      break;
    }
    const [repo, ref, dst, depth, token] = repository
      .split("|")
      .map((item) => item.trim());
    let dirExist = false;
    ...
    try {
      let data = await fs.promises.readFile(`${dstPath}/.git/config`, {
        encoding: "utf8",
      });
      if (token && typeof token !== "undefined") {
        data = data.replace(
          new RegExp("url\\s?=\\s?https://(.+@)?github.com", "g"),
          `url = https://${token}@github.com`,
        );
        await fs.promises.writeFile(`${dstPath}.git/config`, data, {
          encoding: "utf8",
        });
      }
      dirExist = true;
    ...
    let gitArgs;
    if (dirExist) {
      let HEAD = await fs.promises.readFile(`${dstPath}/.git/HEAD`, {
        encoding: "utf8",
      });
      let branch = "";
      try {
